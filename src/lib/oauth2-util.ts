 ...
 */
async function getTokens(callbackUrl: string, code: string): Promise<OAuthToken> {
...
export async function processGoogleOAuth2Callback(
    config: { googleClientId?: string, googleClientSecret?: string, callbackUrl?: string },
    code: string
): Promise<OAuthToken> {
    ...
    if (!googleClientId) throw new Error("Missing googleClientId parameter or environmental variable");
    if (!googleClientSecret) throw new Error("Missing googleClientSecret parameter or environmental variable");
    if (!callbackUrl) throw new Error("Missing callbackUrl parameter or environmental variable");
    ...
    if (!tokenPayload.email) throw new Error("Email not found in token payload");
 ...
 */
async function renewToken(googleClientId: string, refreshToken: string): Promise<OAuthToken> {
    const clientSecret = process.env.FIREBASE_WEB_APP_GOOGLE_CLIENT_SECRET;

    if (!clientSecret) throw new Error('Missing FIREBASE_WEB_APP_GOOGLE_CLIENT_SECRET');
    ...
    if (!process.env.GOOGLE_TOKEN_URL) throw new Error('Missing GOOGLE_TOKEN_URL environment variable');
    ...
    console.info("**renewToken** token renewed with access_token:\n", newToken.access_token, "\nid_token:\n", newToken.id_token);
    return newToken;
}
 ...
 */
export async function createOAuthUrl(oauthSetting: { googleClientId?: string, callbackUrl?: string, scopes: string[] }): Promise<string> {
...
export async function getAccessToken(userId: string) {
    ...
    if (!user) {
        throw new UserAuthError(UserAuthError.ErrorType.NO_RECORD, `**getAccessToken** security profile not found for "${userId}".`);
    }

    if (!user.latest_token || !user.latest_token_id) {
        throw new UserAuthError(UserAuthError.ErrorType.NO_RECORD, `**getAccessToken** lastest token not found for "${userId}".`);
    }

    const nowInSeconds = Math.floor(Date.now() / 1000) + BUFFER_SECONDS;

    if (user.latest_token.access_token && user.latest_token.expires_at && user.latest_token.expires_at > nowInSeconds) {
        // the access token is still active   
        console.info(`**getAccessToken** access token is still active for "${userId}". Expires at ${new Date(user.latest_token.expires_at * 1000).toLocaleString()} `)
        return user.latest_token.access_token;
    } else { // the access token is not active
        console.info(`**getAccessToken** access token expired for "${userId}". Expired at ${new Date((user.latest_token.expires_at || 0) * 1000).toLocaleString()}`)

        if (!user.latest_token.refresh_token || !user.latest_token.refresh_token_expires_at){
            throw new UserAuthError(UserAuthError.ErrorType.NO_RECORD, `**getAccessToken** missing refresh_token or refresh_token_expires_at for "${userId}".`);
        }

        if (user.latest_token.refresh_token_expires_at > nowInSeconds) {
            // we can refresh
            
            const newTokens = await renewToken(process.env.FIREBASE_WEB_APP_GOOGLE_CLIENT_ID,
                user.latest_token.refresh_token);
            
            console.info(`**getAccessToken** renewed token for "${userId}". New access token expires at ${new Date((newTokens.expires_at || 0) * 1000).toLocaleString()}`)

            const tokensForUpdate = shallowCopyObjProperties(newTokens,
                ['access_token', 'expires_in', 'refresh_token', 'refresh_token_expires_in']
            )
            await updateToken(userId, user.latest_token_id, tokensForUpdate);

            return newTokens.access_token
        } else {
            throw new UserAuthError(UserAuthError.ErrorType.EXPIRED, `**getAccessToken** refresh token expired for user ${userId}`)
        }
